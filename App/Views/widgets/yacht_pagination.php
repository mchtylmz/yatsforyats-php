...
$filter = [];
if (get('region')) {
	$filter['region'] = get('region');
}
if (get('person')) {
	$filter['person'] = get('person');
}
if (get('period')) {
	$filter['period'] = get('period');
}
$page = [
...
<div class="col-12 mt-4 mb-4 text-right" style="display: flex;align-items: end;justify-content: flex-end;">
	...
	<a href="<?=lang_site_url(lang_text('yatlar', 'yachts', 'yakhty') . '?' . http_build_query($filter))?>" class="<?=($page['page'] > 1) ? '':'disabled'?>">
  ...
  <?php $filter['page'] = $page['n_page']; ?>
	<a href="<?=lang_site_url(lang_text('yatlar', 'yachts', 'yakhty') . '?' . http_build_query($filter))?>" class="<?=($total > ($page['page'] * $limit) ? '':'disabled')?>">
		<div class="slick-next slick-arrow" style="position: unset;margin: auto 5px;" aria-disabled="false">
